...
@Singleton
class ProductsRepositoryImpl @Inject constructor(private val firestore: FirebaseFirestore) :
    ProductRepository {
    ...
    override suspend fun getCakeItems(): Resource<List<ProductItem>> {
        return try {
            if (cakesItems.isNotEmpty()) {
                Resource.Success(cakesItems)
            } else {
